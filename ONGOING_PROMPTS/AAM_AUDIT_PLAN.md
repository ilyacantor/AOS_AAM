# AAM Codebase Audit: RACI-Aligned Fundamental Fix Plan

## Context

AAM (Adaptive API Mesh) sits between AOD (discovery) and DCL (semantic unification) in the AOS pipeline. Per the **RACI v4 matrix** (AOS_MASTER_RACI_UPDATEDv4.pdf), AAM is **A/R (Accountable + Responsible)** for all Pipe Inference — fabric plane ("WHERE pipes live"), modality ("HOW pipes are accessed"), transport kind, entity scope, identity keys, and change semantics. AOD provides **evidence leads (hints)**, not definitive routing.
...
Steps 1-2 produce definitive results. Step 3 uses display name keywords that AOD attached to candidates. Step 4 uses heuristics on endpoint URLs and evidence signals — reusing logic from `inference.py:infer_fabric_plane()`. If none of the 4 steps produce a result, the candidate is flagged as `needs_operator_review` rather than silently failing — giving the operator visibility instead of an opaque failure. This is the key architectural fix: AAM actively attempts inference using all available evidence rather than refusing to act without a definitive AOD hint.

#### Change 0c: Rewrite `find_matching_pipe()` to use the inference cascade
**File:** `app/services/matching_service.py:46-97`

Replace current logic. Remove Strategy 1 (circular vendor match against candidates). Replace Strategy 2's hard gate (`if not aod_plane_hint: return None`) with the inference cascade from Change 0b. The new pipe creation always includes the inferred plane + confidence score + inference source in provenance.
...
### Fix: Parameterized query `query += " LIMIT ?"` with `cursor.execute(query, (limit,))`
